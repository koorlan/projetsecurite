...
public class DatabaseMain {
	...
	public static void main(String[] args) 
	{
		...
		//	RunQuery(stmt1, QUERIES[5]); //On rempli ID_Cred avec une valeur aleatoire dans Cred_Autorise
			...
			RunQuery(stmt1, QUERIES[7]); //On cree la colonne Liste_Groupe dans la table Utilisateurs
			...
			// 2 - GENERATION DES CLES PUBLIQUES/PRIVEES ET STOCKAGE DANS LA TABLE Cles
			//--------------------------------------------------------------------------------
			...
			while (rss.next()==true){
				...
				byte[] encodedEncryptedValue = TestCipher.cryptWithAes(rss.getString("Valeur").getBytes(),TestCipher.decodeAES_KEY(aesKey));
			...
			// 6 - CHIFFREMENT DES METADONNEES AVEC LES KSec DANS LA TABLE Type_Utili
			//--------------------------------------------------------------------------------
			...
			while (rss.next()==true){
				...
				byte[] encodedEncryptedValue = TestCipher.cryptWithAes(Metadonnees.getBytes(), TestCipher.decodeAES_KEY(aesKey));
			...
			// 7 - CALCUL DE E_Cred_KSec: CHIFFREMENT DE KSec avec les Kpub (CREDENTIALS) DANS LA TABLE Types_Utili
			//-------------------------------------------------------------------------------------------------------
			
			System.out.println("Chiffrement des Ksec avec les Kpub RSA (crédentials)...\n");

			RunQuery(stmt1, QUERIES[29]);// Cree une version de Cles_Types avec KSec
			...
			while (rss.next()==true){
				...
				byte[] cleSecChiffree = TestCipher.decodeAES_KEY(Ksec).getEncoded();  //lol jer crois que �a sert a rien la
				...
				while (creds.hasMoreTokens()) {
					...
					if (firstCrypt){
						cleSecChiffree2 = TestCipher.cryptWithRSApadding(cleSecChiffree, TestCipher.decodeRSA_KEYS(ss, null).getPublic());
						firstCrypt = false;}
					else 
						cleSecChiffree2 = TestCipher.cryptWithRSA(cleSecChiffree, TestCipher.decodeRSA_KEYS(ss, null).getPublic());
			...
			RunQuery(stmt1, QUERIES[4]); //On rajoute une colonne ID_Cred a Cles_Types
			RunQuery(stmt1, QUERIES[5]); //On rempli ID_Cred avec une valeur aleatoire dans Cles_Types
			
			
			//scanner.nextLine();

			// 8 - CREATION D'UNE BD TEMPORAIRE
			//------------------------------------------------------------------------------------------------------
			
			// On copie les tables contenant les users et les frontales dans une BD TEMP
			...
			ResultSet rs2 = stmt4.executeQuery(QUERIES[39]); // Recupere les Frontales
			...
			while (rs2.next()==true){
				...
				if (i==0){
					System.out.println("\nC"
							+ "opie des tables Frontales et Server :");
					//scanner.nextLine();
				}
			...
			// 12 - CREATION DE LA BASE POUR LE LANCEMENT
			//-------------------------------------------------------------------------------------------------------
			...
			// 13 - SUPPRESSION DES BASES INIT_DB COMPLETE ET DE LA BASE TEMP
			//-------------------------------------------------------------------------------------------------------
